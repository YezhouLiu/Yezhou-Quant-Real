...
log = get_logger("compute_volume_ratio")
...
def run(*, force: bool = False):
    ...
    try:
        ...
        for tag, window in VOL_RATIO_SPECS:
            log.info(f"== {tag}: window={window} ==")

            state_key = f"factor:volume_ratio:{window}:v1"
            ...
            if actual_start > req_end:
                log.info("[volume_ratio] already up to date, skip")
                continue
            ...
            for instrument_id in instrument_ids:
                try:
                    ...
                    failed += 1
                    log.warning(f"[volume_ratio] instrument {instrument_id} failed: {e}")

            log.info(
                f"[volume_ratio] {tag} finished: wrote={total_written}, "
