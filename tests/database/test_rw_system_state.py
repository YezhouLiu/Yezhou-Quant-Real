...
class TestSetState:
    ...
    def test_set_dict_state(self, mock_conn):
        ...
        set_state(conn, key='config', value={'version': '1.0', 'debug': True})
    ...
    def test_set_list_state(self, mock_conn):
        ...
        set_state(conn, key='watchlist', value=['AAPL', 'GOOGL', 'MSFT'])
...
class TestDeleteState:
    ...
    def test_delete_existing_state(self, mock_conn):
        ...
        assert cursor.execute.called
        sql = cursor.execute.call_args[0][0]
        assert 'DELETE FROM system_state' in sql
        params = cursor.execute.call_args[0][1]
        assert 'old_config' in params
