...
def test_roll_start_has_no_value_but_later_has_values(monkeypatch, mock_conn):
    cap = CaptureInsert()
    monkeypatch.setattr(
        "factors.volatility_of_volatility.batch_insert_factor_values", cap
    )
    ...
    monkeypatch.setattr(
        "factors.volatility_of_volatility.get_prices", fake_get_prices
    )
...
def test_buffer_is_required(monkeypatch, mock_conn):
    ...
    df = pd.DataFrame(
        {
            "instrument_id": 1,
            "date": dates,
            "adj_close": [100.0 + (i % 5) * 0.5 for i in range(len(dates))],
        }
    )
    ...
    _ = calc_single_instrument_volatility_of_volatility(
        mock_conn,
        instrument_id=1,
        start_date="2005-01-01",
        end_date="2006-01-01",
...
def test_empty_prices_returns_0(monkeypatch, mock_conn):
    monkeypatch.setattr(
        "factors.volatility_of_volatility.get_prices",
        lambda *args, **kwargs: pd.DataFrame(),
    )
    monkeypatch.setattr(
        "factors.volatility_of_volatility.batch_insert_factor_values",
