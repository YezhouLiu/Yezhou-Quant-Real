...
def insert_instrument(conn, ticker: str, exchange: str = 'US', asset_type: str = 'Stock',
                     currency: str = 'USD', company_name: str = None, description: str = None,
                     sector: str = None, industry: str = None, ipo_date: str = None,
                     delist_date: str = None, status: str = 'active', is_tradable: bool = False) -> int:
    """插入资产"""
    cursor = conn.cursor()
    cursor.execute("""
    ...
    """, (ticker, exchange, asset_type, currency, company_name, description, sector,
          industry, ipo_date, delist_date, status, is_tradable))
...
def get_instrument_by_id(conn, instrument_id: int) -> Optional[Dict]:
    ...
    return {
        'instrument_id': row[0], 'ticker': row[1], 'exchange': row[2], 'asset_type': row[3],
        'currency': row[4], 'company_name': row[5], 'description': row[6], 'sector': row[7],
        'industry': row[8], 'ipo_date': row[9], 'delist_date': row[10], 'status': row[11],
        'is_tradable': row[12], 'created_at': row[13], 'updated_at': row[14]
    }


def get_all_instruments(conn, asset_type: str = None, is_tradable: bool = None) -> pd.DataFrame:
    """获取所有资产"""
    ...
    for inst in instruments:
        cursor.execute("""
            ...
            ON CONFLICT (ticker, exchange) DO UPDATE SET
                ...
                updated_at = now()
        """, (
            inst['ticker'], inst.get('exchange', 'US'), inst.get('asset_type', 'Stock'),
            inst.get('currency', 'USD'), inst.get('company_name'), inst.get('description'),
            inst.get('sector'), inst.get('industry'), inst.get('ipo_date'),
            inst.get('status', 'active'), inst.get('is_tradable', False)
        ))
