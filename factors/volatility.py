...
log = get_logger("factor_volatility")


def _infer_factor_name(window: int, annualize: int) -> str:
    return f"vol_{window}d_ann{annualize}"
...
def calc_single_instrument_volatility(
    ...
    *,
    ...
    factor_name = _infer_factor_name(window, annualize)
    factor_args = {"window": window, "annualize": annualize}
    ...
    try:
        ...
        log.warning(f"[volatility] instrument={instrument_id} get_prices failed: {e}")
        return 0
    ...
    if missing:
        log.warning(
            f"[volatility] instrument={instrument_id} missing columns: {sorted(missing)}"
        )
        return 0
    ...
    try:
        ...
        log.warning(f"[volatility] instrument={instrument_id} parse failed: {e}")
        return 0
    ...
    # clean invalid values (price <= 0 -> -inf / nan)
    logp = pd.Series(logp).replace([np.inf, -np.inf], np.nan)

    ret = logp.diff()

    df["factor_value"] = ret.rolling(window).std() * np.sqrt(float(annualize))
    ...
    if valid_rows.empty:
        if len(target) >= 200:
            ...
            log.debug(
                f"[volatility] instrument={instrument_id} produced 0 rows "
    ...
    try:
        ...
        log.warning(f"[volatility] instrument={instrument_id} db write failed: {e}")
        try:
            conn.rollback()
        except Exception:
            pass
