...
log = get_logger("factor_max_drawdown")


def _infer_factor_name(window: int) -> str:
    return f"mdd_{window}d"


def calc_single_instrument_max_drawdown(
    conn: Connection,
    instrument_id: int,
    start_date,
    end_date,
    *,
    ...
    try:
        ...
        log.warning(f"[mdd] instrument={instrument_id} get_prices failed: {e}")
        return 0
    ...
    if missing:
        log.warning(
            f"[mdd] instrument={instrument_id} missing columns: {sorted(missing)}"
        )
        return 0
    ...
    try:
        ...
        log.warning(f"[mdd] instrument={instrument_id} parse failed: {e}")
        return 0

    df = df.sort_values("date")

    # rolling peak
    roll_max = df["price"].rolling(window).max()

    # drawdown at each t
    drawdown = df["price"] / roll_max - 1.0

    # rolling max drawdown (most negative)
    df["factor_value"] = drawdown.rolling(window).min()
    ...
    if valid_rows.empty:
        if len(target) >= 200:
            ...
            log.debug(
                f"[mdd] instrument={instrument_id} produced 0 rows "
    ...
    try:
        ...
        log.warning(f"[mdd] instrument={instrument_id} db write failed: {e}")
        try:
            conn.rollback()
        except Exception:
            pass
